...
{   
    ...
    let navigate=useNavigate();
     ...
     {
        supportCred.current[property]=value;
     }

     function login()
     {
           fetch('http://localhost:8000/support/login',{
                method:"POST",
                headers:{
                    "Content-Type":"application/json"
                },
                body:JSON.stringify(supportCred.current)
           })   
           .then((response)=>response.json())
           .then((data)=>{
                  if(data.success===true)
                  { 
                     localStorage.setItem('medplus_support',JSON.stringify(data));
                     setMessage(null);
